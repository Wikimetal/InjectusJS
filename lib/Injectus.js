...
function Injectus()
{
  ...
  this.resolvedStrings = new Array();
};
...
Injectus.prototype.Resolve = function (value) {
    ...
    var componentInjectusName = InjectusName(value);
    if (!componentInjectusName)
        throw "Invalid component to resolve";
    var componentToResolve = this.components[componentInjectusName];
    if (!componentToResolve)
        throw "No component registered for interface " + componentInjectusName;
    if (this.singletonInstances[componentInjectusName])
        return this.singletonInstances[componentInjectusName];
    this.metrics.Resolve(componentToResolve);
    var implementationName = InjectusName(componentToResolve.Implementation);
    var resolveString = this.resolvedStrings[implementationName];
    
    if(resolveString == null)
    {
      resolveString = "new " + implementationName + "(";
      ...
      resolveString += ")";
      this.resolvedStrings[implementationName] = resolveString;
    }
    if (componentToResolve.LifeStyleType == LifeStyleType.Singleton) {
        var instance = eval(resolveString);
        this.singletonInstances[componentInjectusName] = instance;
        return instance;
    }
